...
void loop() {
  ...
  if (int_count == 1000) { // SLOW LOOP (1Hz)
    ...
    switch (state_num) { // STATE MACHINE (see diagram)
      ...
      case 2: { // Charge Rest, green LED is off and no current
          current_ref = 0;
          if (rest_timer < 360) { // Stay here if timer < 30
            next_state = 2;
            digitalWrite(8, false);
            rest_timer++;
      ...
      case 3: { //Discharge state (-250mA and no LEDs)
          current_ref = -250;
          soc = soc + (current_measure/1908000)*100;
          if ((V_Bat > 2500) && (int(soc*10) != 901) && (int(soc*10) != 801) && (int(soc*10) != 701) && (int(soc*10) != 601) && (int(soc*10) != 501) && (int(soc*10) != 401) && (int(soc*10) != 301) && (int(soc*10) != 201) && (int(soc*10) != 101)) { // While not at minimum volts, stay here
            next_state = 3;
            digitalWrite(8, false);
          }
          else if ((int(soc*10) == 901) || (int(soc*10) == 801) || (int(soc*10) == 701)|| (int(soc*10) == 601)|| (int(soc*10) == 501)|| (int(soc*10) == 401)|| (int(soc*10) == 301)|| (int(soc*10) == 201) || (int(soc*10) == 101)) {
            next_state = 6;
            digitalWrite(8, true);
          }
  ...
  case 6: { // Discharge rest, no LEDs no current
      current_ref = 0;
      if (rest_timer < 360) { // Rest here for 30s like before
        next_state = 6;
        digitalWrite(8, false);
        rest_timer++;
      } else { // When thats done, move back to discharging (and light the green LED)
        next_state = 3;
        digitalWrite(8, true);
        rest_timer = 0;
        soc = soc - 0.1;
