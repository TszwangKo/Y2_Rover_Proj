...
int Travelled = 0;// controlling the turning angle
int angle =0;
int anglenow =0;
...
unsigned long currentMillis = millis();
  ...
  #endif
   //moving forwards
  if (Travelled > total_y1/157) {
    DIRRstate = HIGH;
    DIRLstate = LOW;

  analogWrite(pwmr, spd);       //setting right motor speed at maximum
  analogWrite(pwml, spd);       //setting left motor speed at maximum
      digitalWrite(DIRR, DIRRstate);
    digitalWrite(DIRL, DIRLstate); 
    
  }
    //moving backwards
  else if (Travelled < total_y1/157) {
    DIRRstate = LOW;
    DIRLstate = HIGH;
...
//    }
  ...
  else if(angle<0) //rotating counter clockwise
  {
       angle+=1;
    DIRRstate = LOW;
    DIRLstate = LOW;
...
Serial.println(sqrt(abs(posx*posx)+abs(posy*posy))*10);
 Serial.print('\n');

    
//  //rotating clockwise
//  if (Travelled > f_i && currentMillis <r_i) {
//    DIRRstate = HIGH;
//    DIRLstate = HIGH;
//      digitalWrite(DIRR, DIRRstate);
//    digitalWrite(DIRL, DIRLstate); 
